...
from .calculator import create_attrs_from_setting
from ..network.utils import flatten_recurrent_dict, path_to_links
...
class Controller:
    ...
    def __init__(
        ...
        self.link_latency_attrs = [
            l_attr for l_attr in self.all_link_attrs if l_attr.type == "latency"
        ]
    ...
    def check_attributes(self, v: dict, p: dict, attrs_list: list) -> Tuple[bool, dict]:
        ...
        for attr in attrs_list:
            ...
            if attr.type == "resource":
                ...
                satisfiability_info[attr.name] = 0.0

        return final_result, satisfiability_info

    def check_graph_constraints(
        self, v_net: VirtualNetwork, p_net: PhysicalNetwork
    ) -> Tuple[bool, dict]:
        """
        ...
        """
        final_result, graph_satisfiability_info = self.check_attributes(
            v_net, p_net, self.all_node_attrs
        )
    ...
    def check_node_constraints(
        ...
        final_result, node_satisfiability_info = self.check_attributes(
            v_node, p_node, self.all_node_attrs
        )
    ...
    def check_link_constraints(
        ...
        final_result, link_satisfiability_info = self.check_attributes(
            v_link, p_link, self.all_link_attrs
        )

        return final_result, link_satisfiability_info

    def check_path_constraints(
        self, v_net: VirtualNetwork, p_net: PhysicalNetwork, v_link: dict, p_path: list
    ) -> Tuple[bool, dict]:
        """
        ...
        if operator in ["+", "add"]:
            ...
            if element_owner == "node":
                if safe:
                    assert (
                        net.nodes[element_id][attr_name] >= value
                    ), f"Node {element_id} and Attribute {attr_name}: {net.nodes[element_id][attr_name]} - {value}"
                net.nodes[element_id][attr_name] -= value
            elif element_owner == "link":
                if safe:
                    assert net.links[element_id][attr_name] >= value
                net.links[element_id][attr_name] -= value
        else:
    ...
    def update_link_resources(
        ...
        p_net: PhysicalNetwork,
        p_link: int,
        used_link_resources: dict,
        operator: Optional[str] = "-",
        safe: Optional[bool] = True,
    ):
        for e_attr_name, value in used_link_resources.items():
            self.update_resource(
                p_net, "link", p_link, e_attr_name, value, operator=operator, safe=safe
            )

    def update_path_resources(
        self,
        v_net: VirtualNetwork,
        p_net: PhysicalNetwork,
        v_link,
    ...
    def safely_place(
        ...
        """
        check_result, check_info = self.check_node_constraints(
            v_net, p_net, v_node_id, p_node_id
        )
        if not check_result:
            return False, check_info

        used_node_resources = {
            n_attr.name: v_net.nodes[v_node_id][n_attr.name]
    ...
    def unsafely_place(
        ...
        self.update_node_resources(
            p_net, p_node_id, used_node_resources, operator="-", safe=False
        )
        solution["node_slots"][v_node_id] = p_node_id
        solution["node_slots_info"][(v_node_id, p_node_id)] = used_node_resources
        ...
        if check_result:
            ...
            all_satisfiability_value_list = flatten_recurrent_dict(check_info)
            all_violation_value_list = [
                v if v < 0.0 else 0 for v in all_satisfiability_value_list
            ]
        ...
        solution["v_net_single_step_violation"] = -100.0

        return True, check_info

    def undo_place(
        self, v_node_id: int, p_net: PhysicalNetwork, solution: Solution
    ) -> bool:
        """
    ...
    def safely_route(
        ...
        check_info = {l_attr.name: 0.0 for l_attr in v_net.get_link_attrs()}
        ...
        shortest_paths = self.find_shortest_paths(
            v_net, p_net, v_link, pl_pair, method=shortest_method, k=k
        )
        shortest_paths = (
            rank_path_func(v_net, p_net, v_link, pl_pair, shortest_paths)
            if rank_path_func is not None
            else shortest_paths
        )
        check_info_list = []
        for p_path in shortest_paths:
            check_result, check_info = self.check_path_constraints(
                v_net, p_net, v_link, p_path
            )
            if check_result:
                p_links = path_to_links(p_path)
                ...
                for p_link in p_links:
                    used_link_resources = {
                        l_attr.name: v_net.links[v_link][l_attr.name]
                        for l_attr in self.link_resource_attrs
                    }
                    self.update_link_resources(
                        p_net, p_link, used_link_resources, operator="-", safe=True
                    )
    ...
    def unsafely_route(
        ...
        pruned_p_net = (
            ...
            else self.create_pruned_network(
                v_net, p_net, v_link_pair=v_link, ratio=pruning_ratio, div=0
            )
        )
        shortest_paths = self.find_shortest_paths(
            v_net, pruned_p_net, v_link, pl_pair, method=shortest_method, k=k
        )
        if len(shortest_paths) == 0:
            solution["v_net_single_step_violation"] = 0.0
            return False, check_info
        check_result_list = []
        check_info_list = []
        ...
        for p_link in p_links:
            ...
            self.update_link_resources(
                p_net, p_link, used_link_resources, operator="-", safe=False
            )
            solution["link_paths_info"][(v_link, p_link)] = used_link_resources
    ...
    def undo_route(
        self, v_link: tuple, p_net: PhysicalNetwork, solution: Solution
    ) -> bool:
        """
    ...
    def place_and_route(
        ...
        """
        if check_feasibility:
            return self.safely_place_and_route(
                v_net,
                p_net,
                v_node_id,
                p_node_id,
            ...
            return self.unsafely_place_and_route(
                v_net,
                p_net,
                v_node_id,
                p_node_id,
    ...
    def safely_place_and_route(
        ...
        """
        # Place
        place_result, place_info = self.place(
        ...
        # Route
        route_info = {l_attr.name: 0.0 for l_attr in v_net.get_link_attrs()}
        to_route_v_links = []
        v_node_id_neighbors = list(v_net.adj[v_node_id])
        for n_v_node_id in v_node_id_neighbors:
            placed = (
                n_v_node_id in solution["node_slots"].keys()
                and solution["node_slots"][n_v_node_id] != -1
            )
            routed = (n_v_node_id, v_node_id) in solution["link_paths"].keys() or (
                v_node_id,
                n_v_node_id,
            ) in solution["link_paths"].keys()
            if placed and not routed:
                to_route_v_links.append((v_node_id, n_v_node_id))

        if shortest_method == "mcf":
            route_result, route_info = self.route_v_links_with_mcf(
                v_net, p_net, to_route_v_links, solution
            )
            if not route_result:
                # FAILURE
    ...
    def unsafely_place_and_route(
        ...
        """
        # Place
        place_result, place_info = self.unsafely_place(
            v_net, p_net, v_node_id, p_node_id, solution
        )
        ...
        for v_link in to_route_v_links:
            n_p_node_id = solution["node_slots"][v_link[1]]
            route_result, route_info = self.unsafely_route(
                v_net,
                p_net,
                v_link,
                (p_node_id, n_p_node_id),
        ...
        route_violation_value_of_all_paths = (
            sum(route_violation_value_list)
            if len(route_violation_value_list) > 0
            else -100.0
        )
        # best_violation_value = place_violation_value + sum(route_violation_value_list)
        best_violation_value = max(
            place_violation_value, route_violation_value_of_all_paths
        )
        solution["v_net_single_step_violation"] = best_violation_value
    ...
    def node_mapping(
        ...
        """
        if check_feasibility:
            return self.safely_node_mapping(
                v_net,
                p_net,
                sorted_v_nodes,
                sorted_p_nodes,
    ...
    def safely_node_mapping(
        ...
        for v_node_id in sorted_v_nodes:
            for p_node_id in sorted_p_nodes:
                ...
                if place_result:
                    if reusable == False:
                        sorted_p_nodes.remove(p_node_id)
                    break
                else:
    ...
    def link_mapping(
        ...
        """
        if check_feasibility:
            return self.safely_link_mapping(
                v_net, p_net, solution, sorted_v_links, shortest_method, k, inplace
            )
        else:
            return self.unsafely_link_mapping(
                v_net, p_net, solution, sorted_v_links, shortest_method, k, inplace
            )
    ...
    def safely_link_mapping(
        ...
        p_net = p_net if inplace else copy.deepcopy(p_net)
        sorted_v_links = (
            sorted_v_links if sorted_v_links is not None else list(v_net.links)
        )
        node_slots = solution["node_slots"]

        if shortest_method == "mcf":
            route_result, route_info = self.route_v_links_with_mcf(
                v_net, p_net, sorted_v_links, solution
            )
            if not route_result:
                # FAILURE
                solution.update({"route_result": False, "result": False})
        ...
        # SUCCESS
        assert (
            len(solution["link_paths"]) == v_net.num_links
        ), f"Number of total links: {v_net.num_links}, Number of routed links: {len(solution['link_paths'])}"
    ...
    def unsafely_link_mapping(
        ...
        for v_link_pair in sorted_v_links:
            ...
            route_result, route_check_info = self.unsafely_route(
                v_net,
                p_net,
                v_link_pair,
                p_path_pair,
    ...
    def deploy(
        ...
        if not solution["result"]:
            return False
        for (v_node_id, p_node_id), used_node_resources in solution[
            "node_slots_info"
        ].items():
            self.update_node_resources(
                p_net, p_node_id, used_node_resources, operator="-"
            )
        for (v_link, p_link), used_link_resources in solution[
            "link_paths_info"
        ].items():
            self.update_link_resources(p_net, p_link, used_link_resources, operator="-")
    ...
    def bfs_deploy(
        ...
        while queue:
            ...
            place_and_route_reult, place_and_route_info = self.place_and_route(
                v_net,
                p_net,
                v_node_id,
                curr_pid,
            ...
            if place_and_route_reult:
                ...
                if v_node_id in solution["node_slots"]:
                    self.undo_place_and_route(
                        v_net, p_net, v_node_id, curr_pid, solution
                    )
            ...
            node_links = (
                node_links
                if len(node_links) <= max_visit
                else list(node_links)[:max_visit_in_every_depth]
            )
    ...
    def deploy_with_node_slots(
        ...
        """
        if check_feasibility:
            return self.safely_deploy_with_node_slots(
                v_net, p_net, node_slots, solution, inplace, shortest_method, k_shortest
            )
        else:
            return self.unsafely_deploy_with_node_slots(
                v_net, p_net, node_slots, solution, inplace, shortest_method, k_shortest
            )
    ...
    def safely_deploy_with_node_slots(
        ...
        if len(node_slots) != v_net.num_nodes or -1 in node_slots.values():
            ...
            return
        # node mapping
        node_mapping_result = self.node_mapping(
            v_net,
            p_net,
        ...
        if not node_mapping_result:
            ...
            return
        # link mapping
        link_mapping_result = self.link_mapping(
            v_net,
            p_net,
            solution,
            sorted_v_links=None,
    ...
    def unsafely_deploy_with_node_slots(
        ...
        if not node_mapping_result:
            ...
            return
        # link mapping
        link_mapping_result, route_check_info = self.unsafely_link_mapping(
            v_net,
            p_net,
            solution,
            sorted_v_links=None,
    ...
    def find_shortest_paths(
        ...
        try:
            ...
            if method == "first_shortest":
                shortest_paths = [
                    nx.dijkstra_path(p_net, source, target, weight=weight)
                ]
            elif method == "k_shortest":
                shortest_paths = list(
                    islice(
                        nx.shortest_simple_paths(p_net, source, target, weight=weight),
                        k,
                    )
                )
                ...
                for path in nx.shortest_simple_paths(
                    p_net, source, target, weight=weight
                ):
                    if len(path) <= k:
                ...
                shortest_paths = list(
                    nx.all_shortest_paths(p_net, source, target, weight=weight)
                )
            # these two methods return a fessible path or empty by considering link constraints
            elif method == "bfs_shortest":
                if weight is not None:
                    raise NotImplementedError(
                        "BFS Shortest Path Method not supports seeking for weighted shorest path!"
                    )
                shortest_path = self.find_bfs_shortest_path(
                    v_net, p_net, v_link, source, target, weight=weight
                )
                ...
                shortest_paths = [
                    nx.dijkstra_path(temp_p_net, source, target, weight=weight)
                ]
    ...
    def create_available_network(
        self, v_net: VirtualNetwork, p_net: PhysicalNetwork, v_link_pair
    ):
    ...
    def route_v_links_with_mcf(
        ...
        for v_link in v_link_list:
            p_pair = (
                solution["node_slots"][v_link[0]],
                solution["node_slots"][v_link[1]],
            )
        ...
        for p_pair in p_pair_list:
            ...
            for p_link in directed_p_graph.edges():
                f[p_pair][p_link] = solver.IntVar(0.0, 9999, f"{p_pair}-{p_link}")
        # set objective
        solver.Minimize(
            sum(
                get_link_weight(p_link)
                * sum(f[p_pair][p_link] for p_pair in p_pair_list)
                for p_link in directed_p_graph.edges
            )
        )
        # set constraints
        ...
        for p_pair in p_pair_list:
            ...
            solver.Add(
                sum(
                    f[p_pair][p_link]
                    for p_link in directed_p_graph.out_edges(nbunch=source_node)
                )
                == request_dict[p_pair]
            )
            solver.Add(
                sum(
                    f[p_pair][p_link]
                    for p_link in directed_p_graph.in_edges(nbunch=source_node)
                )
                == 0
            )
            solver.Add(
                sum(
                    f[p_pair][p_link]
                    for p_link in directed_p_graph.in_edges(nbunch=target_node)
                )
                == request_dict[p_pair]
            )
            solver.Add(
                sum(
                    f[p_pair][p_link]
                    for p_link in directed_p_graph.out_edges(nbunch=target_node)
                )
                == 0
            )
            for node in directed_p_graph.nodes:
                ...
                solver.Add(
                    sum(
                        f[p_pair][p_link]
                        for p_link in directed_p_graph.out_edges(nbunch=node)
                    )
                    - sum(
                        f[p_pair][p_link]
                        for p_link in directed_p_graph.in_edges(nbunch=node)
                    )
                    == 0
                )
        ...
        if status == pywraplp.Solver.OPTIMAL or status == pywraplp.Solver.FEASIBLE:
            ...
            for e in p_net.links:
                ...
                s_2 = sum(
                    [f[p_pair][e[1], e[0]].solution_value() for p_pair in p_pair_list]
                )
                ...
                if total_link_resource == 0:
                    ...
                    for p_link in directed_p_graph.edges():
                        if f[p_pair][p_link].solution_value():
                            used_link_resources = {
                                self.link_resource_attrs[0]
                                .name: f[p_pair][p_link]
                                .solution_value()
                            }
                            solution["link_paths_info"][
                                (v_link, p_link)
                    ...
                    all_shortest_paths = list(
                        nx.shortest_simple_paths(G, p_pair[0], p_pair[1])
                    )
                    source_p_links = set(
                        [(path[0], path[1]) for path in all_shortest_paths]
                    )
                    target_p_links = set(
                        [(path[-2], path[-1]) for path in all_shortest_paths]
                    )
                    assert (
                        sum(
                            [
                                f[p_pair][source_p_link].solution_value()
                                for source_p_link in source_p_links
                            ]
                        )
                        == total_link_resource
                    )
                    assert (
                        sum(
                            [
                                f[p_pair][target_p_link].solution_value()
                                for target_p_link in target_p_links
                            ]
                        )
                        == total_link_resource
                    )
    ...
    def find_bfs_shortest_path(
        ...
        while len(Q) and not found_target:
            ...
            for neighbor in nx.neighbors(p_net, current_node):
                check_result, check_info = self.check_link_constraints(
                    v_net, p_net, v_link, (current_node, neighbor)
                )
                if check_result:
    ...
    def find_candidate_nodes(
        ...
        if check_node_constraint:
            suitable_nodes = [
                ...
                if self.check_node_constraints(v_net, p_net, v_node_id, p_node_id)[0]
            ]
        ...
        if check_link_constraint:
            ...
            v_link_aggr_resource = np.array(
                v_net.get_aggregation_attrs_data(
                    self.link_resource_attrs, aggr=aggr_method
                )
            )
            ...
            resource_comparison = np.all(
                v_link_aggr_resource[:, [v_node_id]] <= p_link_aggr_resource[:, :],
                axis=0,
            )
            suitable_nodes = all_p_nodes[
                np.logical_and(degrees_comparison, resource_comparison)
            ]
    ...
    def find_feasible_nodes(
        self, v_net: VirtualNetwork, p_net: PhysicalNetwork, v_node_id, node_slots
    ):
        """
        ...
        node_constraints_feasible_nodes = list(
            set(node_constraints_feasible_nodes).difference(
                set(list(node_slots.values()))
            )
        )
        ...
        for v_neighbor_id, p_neighbor_id in node_slots.items():
            if v_neighbor_id not in v_net.adj[v_node_id]:
                continue
            temp_p_net = self.create_available_network(
                v_net, p_net, (v_neighbor_id, v_node_id)
            )
            new_feasible_nodes = []
            for p_node_id in feasible_nodes:
                try:
                    shortest_paths = [
                        nx.dijkstra_path(temp_p_net, p_neighbor_id, p_node_id)
                    ]
