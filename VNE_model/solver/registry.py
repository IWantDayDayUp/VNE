...
from ..simulation.environment import SolutionStepEnvironment
...
class Registry:
    ...
    def __repr__(self):
        return f"{self.__class__.__name__ }(name={self._name}, items={list(self._solver_dict.keys())})"
    ...
    def get(self, solver_name: str) -> dict:
        ...
        if solver_name not in self._solver_dict.keys():
            raise KeyError(
                f"The solver {solver_name} is not in the {self.name} registry"
            )
    ...
    def _add(
        ...
        if not inspect.isclass(solver_cls) and not inspect.isclass(env_cls):
            raise TypeError(
                f"module must be a class, but got {type(solver_cls)} and {type(env_cls)}"
            )
        if solver_name in self._solver_dict:
            raise KeyError(f"{solver_name} is already registered in {self.name}")

        self._solver_dict[solver_name.lower()] = {
            "solver": solver_cls,
            "env": env_cls,
            "type": solver_type.lower(),
        }
