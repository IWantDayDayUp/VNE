...
- 命名约定: 在类内部, 变量或方法以 '_' 开头('_name'或'_get_name()'), 不建议在类外部直接访问
- 避免命名冲突: 当我们在导入模块时, 可以使用单下划线作为前缀, 以避免与当前命名空间中的其他标识符冲突
- 临时变量: 只需要临时使用某个变量, 而不关心它的具体值, 如: `for _ in range(5):`
- 引入未使用的变量: 我们可能会定义一些变量, 但在后续的代码中并没有使用它们, 如: `_, y, _ = (1, 2, 3)`

### 双下划线 '__' 使用

- 名称修饰: 即将属性或方法前面的双下划线作为前缀, 使其成为私有成员, 只能在类内部访问, 无法在类外部直接访问
- 魔术方法: 魔术方法是一种特殊的方法, 用于在特定情况下自动调用, 从而实现对对象的特定行为进行定制, 如: `__str__(self)`
